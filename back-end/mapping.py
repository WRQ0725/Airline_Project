...
def extract_cities(section):
    for item in section:
        ...
        city_code = data[start:end]
        # 添加到映射表
        city_code_to_name[city_code] = display_name

# 提取热门城市
extract_cities(data["热门"])

# 提取其他城市
for letter_section in data["ABCDEF"].values():
    extract_cities(letter_section)

# 提取其他部分（如 GHIJ, KLMN 等）
for key, value in data.items():
    if isinstance(value, dict):
        for letter_section in value.values():
            extract_cities(letter_section)

# 打印映射表
print(json.dumps(city_code_to_name, ensure_ascii=False, indent=4))